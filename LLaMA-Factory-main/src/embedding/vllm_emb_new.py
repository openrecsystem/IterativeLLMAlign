...
def embedding():
    ...
    model = LLM(model=args.model, trust_remote_code=True, gpu_memory_utilization=0.9, enforce_eager=True, tensor_parallel_size=1)
    ...
    embeddings = np.round(np.array(embeddings), decimals=4)
    for i in range(data_L):
        datas['emb'][i] = embeddings[:][i]
        
    with open(args.emb_save_file, 'wb') as resfile:
        pickle.dump(datas[['User ID', 'Movie ID','emb']], resfile)
